...
async function playAttempt(host, attemptIdx) {
  return new Promise((resolve) => {
    ...
    function hasAvailableOdd() {
      for (let i = 0; i < DICE; i++) {
        if (!frozen.has(i) && values[i] != null && values[i] % 2 === 1) return true;
      }
      return false;
    }
    ...
    function countOdds() {
      let c = 0;
      for (let i = 0; i < DICE; i++) {
        if (!frozen.has(i) && values[i] != null && values[i] % 2 === 1) c += 1;
      }
      return c;
    }
    ...
    function render() {
      for (let i = 0; i < DICE; i++) {
        ...
        const isOdd = values[i] != null && values[i] % 2 === 1;
        applyState(d, {
          frozen: isFrozen,
          staged: isStaged,
          dim: !isFrozen && !isOdd,
          selectable: !isFrozen && isOdd,
        });
      }
      renderControls();
    }
    ...
    async function initialRoll() {
      ...
      if (!hasAvailableOdd()) {
        status.innerHTML = `No odd dice rolled — attempt invalid.`;
...
export default {
  ...
  decathlon: 'reiner',
  name: 'Javelin',
  blurb: 'Roll 6 dice; freeze odds only until all 6 are odd.',
  rulesHtml: rules,
  rounds: ATTEMPTS,
